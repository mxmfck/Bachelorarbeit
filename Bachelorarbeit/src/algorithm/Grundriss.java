	...
	private void platziereFensterInKleinenRaumen(RaumModell raum, boolean istLinks, boolean unten) {
		if (unten) {// unten
			...
			if (istLinks) { // Fenster oben //TODO Fenst oben position gut?
				if (raum.getLaenge() < raum.getBreite()) {
					raum.addFenster(new Fenster(raum.getRaumX() + (raum.getRaumLaenge() - 0.8) / 2,
							raum.getY() + raum.getRaumBreite() , 0.8, true));
				} else {// Fenster links
					raum.addFenster(new Fenster(raum.getX() ,
							raum.getRaumY() + (raum.getRaumBreite() - 0.8) / 2, 0.8, false));
				}
				...
				if (raum.getLaenge() < raum.getBreite()) {// Fenster oben
					raum.addFenster(new Fenster(raum.getRaumX() + (raum.getRaumLaenge() - 0.8) / 2,
							raum.getY() + raum.getRaumBreite() + WANDBREITE, 0.8, true));
				} else {// Fenster rechts
					raum.addFenster(new Fenster(raum.getX() + raum.getLaenge() ,
							raum.getRaumY() + (raum.getRaumBreite() - 0.8) / 2, 0.8, false));
				}
...
//	private double maxLinks = 0; // Maximale x-Koordinate der Räume links des Flurs
//	private boolean linksPlaziert; // Gibt an, ob der letzte Raum links oder rechts platziert wurde
//	private static double WANDBREITE = 0.2;
//
//	public Grundriss(Haus haus) {
//		berechneGrundriss(haus);
//
...
//						tueren, 0+WANDBREITE, 0+WANDBREITE));
//				return;
//			}
//		}
//	}
...
//							new TuerModell(tuer.getVonRaum(), tuer.getInRaum(), tuer.getBreite()));
//				}
//				RaumModell raumModell = new RaumModell(raum.getName(), raum.getLaenge(), raum.getBreite(), raum.getMoebel(),
//						tueren, links ? x - raum.getLaenge()-WANDBREITE : x+WANDBREITE, y+WANDBREITE);
//				raeume.add(raumModell);
//				y += raumModell.getBreite();
//				max = links ? raumModell.getLaenge() : Math.round((x + raumModell.getLaenge()*100.0)/100.0);;
//			} else {
//				List<TuerModell> tueren = new ArrayList<>();
//				for (Tuer tuer : raum.getTueren()) {
//					tueren.add(
//							new TuerModell(tuer.getVonRaum(), tuer.getInRaum(), tuer.getBreite()));
//				}
//				RaumModell raumModell = new RaumModell(raum.getName(), raum.getBreite(), raum.getLaenge(), raum.getMoebel(),
//						tueren, links ? x - raum.getBreite() - WANDBREITE : x+WANDBREITE, y+WANDBREITE);
//				raeume.add(raumModell);
//				y += raumModell.getLaenge();
//				max = links ? raumModell.getBreite() : Math.round((x + raumModell.getBreite()*100.0)/100.0);
//			}
//		} else {
//			if (Math.abs(raum.getLaenge() - raeume.get(raeume.size() - 1).getLaenge()) < Math
//					.abs(raum.getBreite() - raeume.get(raeume.size() - 1).getLaenge())) {
...
//				y += raum.getLaenge();
//				if (links) {
//					if (max < raumModell.getBreite()) {
//						max = Math.round((x + raumModell.getBreite()*100.0)/100.0);;
//					}
//				} else {
//					if (max < x + raumModell.getBreite()) {
//						max = Math.round((x + raumModell.getBreite()*100.0)/100.0);
//					}
//				}
//			}
